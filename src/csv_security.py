...
def _safe_reader(stream: BinaryIO):
    """Create a strict UTF-8 CSV reader with a bounded field size."""
    try:
    ...
    try:
        ...
        for row_number, row in enumerate(reader, start=1):
            ...
            if any(len(field) > MAX_CSV_FIELD_LENGTH for field in row):
                raise CSVSecurityError(
                    f"CSV field exceeds the maximum length of {MAX_CSV_FIELD_LENGTH} characters."
                )
    ...
    if len(header) > MAX_CSV_COLUMNS:
        raise CSVSecurityError(f"CSV exceeds the maximum of {MAX_CSV_COLUMNS} columns.")
...
def read_bounded_csv(uploaded_file) -> pd.DataFrame:
    """Parse an untrusted CSV only within configured row/column/field limits."""
