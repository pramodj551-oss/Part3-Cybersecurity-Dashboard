...
def _build_runtime(tmp_path: Path) -> tuple[Path, dict[str, str]]:
    ...
    manifest = {"source_release_tag": "test-release", "files": hashes}
...
def test_manifest_hash_modification_fails_closed(monkeypatch, tmp_path):
    ...
    manifest_path.write_text(json.dumps({"source_release_tag": "test-release", "files": hashes}), encoding="utf-8")
    _patch_runtime(monkeypatch, tmp_path, manifest_path)
...
def test_runtime_loader_refuses_to_deserialize_when_identity_fails(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(model_loader, "verify_runtime_artifact_identity", lambda: (False, {}, "hash mismatch: models/best_model.pkl"))
    load_called = False
...
def test_direct_model_loader_refuses_to_deserialize_when_identity_fails(monkeypatch, tmp_path):
    ...
    # Make these explicit paths the configured production paths for this test;
    # the production-path policy itself remains enforced by ModelLoader.load().
